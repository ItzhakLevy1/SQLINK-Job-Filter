...
/**
 * Enhanced ID extraction logic to handle both visible job numbers
 * and internal system IDs found in the HTML.
 */
function findJobId(jobContainer) {
  // Strategy 1: Look for the visible job number (e.g., 145401)
  const jobIdElement = jobContainer.querySelector(".description.number");
  if (jobIdElement) {
    const textMatch = jobIdElement.innerText.match(/\d+/);
    if (textMatch) return textMatch[0];
  }

  // Strategy 2: Fallback to the internal system ID in HTML (e.g., id-32319)
  const idElement = jobContainer.querySelector('[id^="id-"]');
  if (idElement && idElement.id) {
    const idMatch = idElement.id.match(/\d+/);
...
function extractExperienceYears(text) {
  ...
  const wordPattern = new RegExp(
    `(${wordKeys})(?:\\s+ומעלה|\\s+שנה|\\s+שנות|\\s+שנים)`,
    "g",
  );
...
function addApplyButtonListeners() {
  ...
  applyLinks.forEach((link) => {
    ...
    link.addEventListener("click", function () {
      const jobContainer = this.closest(".positionItem");
      if (!jobContainer) return;

      const jobId = findJobId(jobContainer);
      if (!jobId) return;
